	...
	constructor(config, channel) {
		...
		this.connection.on('error', (error) => {
			if (error === 'connectionerror') {
				...
				channel.send(`\`Desconectouse do servidor de OpenTTD: ${this.name}\``);
				global.logger.info(`OpenTTD connection closed: ${this.name}`);
				this.connection = new openttdAdmin.connection();
				setTimeout(() => this.connection.connect(), 10000);
			}
			if (this.isConnected) {
				this.isConnected = false;
			...
			if (this.clientInfo[id].name) {
				...
				if (this.clientInfo[id].company === 255) {
					...
					if (this.companyInfo[this.clientInfo[id].company]) {
						join += ` na compañía #${this.clientInfo[id].company + 1} (${
							this.companyInfo[this.clientInfo[id].company].name
						})`;
					}
				}
				channel.send(`\`${join}\``);
		...
		this.connection.on('companyremove', (company) => {
			...
			switch (company.reason) {
				case openttdAdmin.enums.CompanyRemoveReasons.MANUAL:
					remove += ' pola Xunta de Galicia';
					break;
				case openttdAdmin.enums.CompanyRemoveReasons.AUTOCLEAN:
					remove += ' por falta de actividade.';
					break;
				case openttdAdmin.enums.CompanyRemoveReasons.BANKRUPT:
					remove += ' despois dun concurso de acreedores';
					break;
			}
			channel.send(`\`${remove}\``);
